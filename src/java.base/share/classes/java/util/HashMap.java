...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        if ((p = tab[i = (n - 1) & hash]) == null)
            ...
            if (e != null) { // existing mapping for key
                V oldValue = e.value;
                /*
                onlyIfAbsent默认false表示当待插入key已经存在时用新的value更新oldValue，否则保持原先的key-value不变
                 */
    ...
    final Node<K,V>[] resize() {
        ...
        int oldCap = (oldTab == null) ? 0 : oldTab.length; // 首次创建table为null，容量为0，否则为table的大小
        int oldThr = threshold; // 首次创建阈值为0（负载因子 * 容量）
        int newCap, newThr = 0;
        /*
         如果旧容量大于0，扩容容量
         */
        if (oldCap > 0) {
            ...
            /*
             容量翻倍（oldCap << 1 == oldCap * 2）并未超过最大容量，且扩容之前的容量大于默认初始容量（16）
             */
            else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY &&
                     oldCap >= DEFAULT_INITIAL_CAPACITY)
                newThr = oldThr << 1; // 阈值翻倍，新阈值 = 旧阈值 * 2,
        }
        else if (oldThr > 0) // 初始化容量等于阈值
            newCap = oldThr;
        /*
        ...
        /*
        如果要调整的阈值为0，容量小于最大容量并且新的阈值小于最大容量，该阈值为newCap * loadFactor，否者为Integer.MAX_VALUE
         */
        if (newThr == 0) {
        ...
        if (oldTab != null) {
            for (int j = 0; j < oldCap; ++j) {
                 ...
                 */
                if ((e = oldTab[j]) != null) {
                    oldTab[j] = null; // 清除旧表的引用
                    /*
                    如果hash表的桶的链表只有一个元素，就直接将它放到新表的目标位置
                     */
                    if (e.next == null)
                     ...
                     */
                        ...
                        do {
                            next = e.next;
                            /*
                            新表是旧表的两倍容量，实例上就把单链表拆分为两队，e.hash & oldCap 为偶数一队，e.hash & oldCap 为奇数一对
    ...
    static final class TreeNode<K,V> extends LinkedHashMap.Entry<K,V> {
        ...
        final Node<K,V> untreeify(HashMap<K,V> map) {
            ...
            for (Node<K,V> q = this; q != null; q = q.next) {
                Node<K,V> p = map.replacementNode(q, null); //树节点转换为链表节点 TreeNode 转换为 Node
                if (tl == null)
                    hd = p;
                else
                    tl.next = p;
